...
describe('test SQLEngine class', () => {
    ...
    test('SELECT id, title FROM movies', () => {
        ...
        let expected = database.movies.map(movie => ({ title: movie.title, id: movie.id }));
        expect(result).toEqual(expected);
    });
    // test('SELECT title FROM movies WHERE id=1', () => {
    //     let engine = new SQLEngine(database);
    //     let result = engine.execute('SELECT title FROM movies WHERE id=1');
    //     let expected = database.movies.filter(movie => movie.id === 1).map(movie => ({ title: movie.title }));
    //     expect(result).toEqual(expected);
    // });
    // test('SELECT title FROM movies WHERE cert >=15', () => {
    //     let engine = new SQLEngine(database);
    //     let result = engine.execute('SELECT title FROM movies WHERE cert >=15');
    //     let expected = database.movies.filter(movie => movie.cert >= 15).map(movie => ({ title: movie.title }));
    //     expect(result).toEqual(expected);
    // });
    // test('SELECT id FROM actors WHERE name = "Keanu Reeves"', () => {
    //     let engine = new SQLEngine(database);
    //     let result = engine.execute('SELECT id FROM actors WHERE name = "Keanu Reeves"');
    //     let expected = database.actors.filter(actor => actor.name === "Keanu Reeves").map(actor => ({ id: actor.id }));
    //     expect(result).toEqual(expected);
    // });
});